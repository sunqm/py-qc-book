...
class String:
    ...
    def __init__(self, operators, factor=1, deltas=None, remaining=None):
        ...
        self.remaining: List[Union[FieldOperator, FOpGroup]] = remaining or []
...
def _contract_fgroup_fop(fgroup: FOpGroup, fop: FieldOperator) -> List[String]:
    degeneracy = len(fgroup)
    op = fgroup[-1]
    if not commutable(fop, op):
...
def _contract_fgroup_fgroup(fgroup1: FOpGroup, fgroup2: FOpGroup) -> List[String]:
    degen1 = len(fgroup1)
    degen2 = len(fgroup2)
...
def _contract_fop_string(fop: Union[FieldOperator, FOpGroup], string: String) -> List[String]:
...
def on_HF_reference(string, tensors, output_indices=None) -> List[str]:
    ...
    deltas = string.deltas
    if h_ops:
        # The remaining dangling operators in Hamiltonian
        if (len(h_ops) == 2 and
