...
@custom_fixed_point(lambda t2, H, *args, **kwargs: update_CCD_amplitudes(H, t2),
                    solve=partial(solve_gmres, tol=1e-5))
...
def mo_integrals_jvp(primals, tangents):
    ...
    return _mo_integrals_base(*primals), _mo_integrals_common(mol, hcore, eri, e_hf)

def _mo_integrals_base(mf):
    mol = mf.mol
    orb = mf.mo_coeff
